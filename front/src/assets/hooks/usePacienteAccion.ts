...
const usePacienteAccion = () => {
  ...
  const deshabilitarPaciente = async (dniPaciente: string): Promise<boolean> => {
    ...
    const indexPaciene = pacientes.findIndex((p) => p.dni === dniPaciente); // Obtengo el paciente por su dni
    if (indexPaciene === -1) return false; // si no se encuentra el paciente, retorno false
    /* await pacientes.splice(paciente.pacienteId, 1); // Elimino el paciente de la lista */
    pacientes[indexPaciene].deshabilitado = true; // Deshabilito el paciente
